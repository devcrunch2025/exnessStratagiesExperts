...
double MinimumSameOrderGapRaw =20;
...
int GetSSLSignal() {
   ...
   if(previousClose <= upper && currentClose > upper) {
      if(InpUseEMA200Filter) {
         if(currentClose > iMA(Symbol(), 0, InpEMA200Period, InpEMAPriceShift, MODE_EMA, PRICE_CLOSE, 0)) return 1;
      } else return 1;
   }
...
void ClearEMALineObjects() {
   for(int i=ObjectsTotal()-1; i>=0; i--) {
      ...
      if(StringFind(name,EMA_PREFIX,0)==0) ObjectDelete(0,name);
   }
}

void UpdateEMALineOnChart() {
   if(!ShowEMALine || Bars < InpEMA200Period + 2) return;
   ...
   if(!rebuild && LastEMARedrawMs!=0 && (uint)(emaNow-LastEMARedrawMs)<(uint)MathMax(0,EMARedrawIntervalMs)) return;
   if(rebuild) {
      ClearEMALineObjects();
      for(int i=barsToDraw-1; i>=0; i--) {
         ...
         if(!ObjectCreate(0,name,OBJ_TREND,0,Time[j],ema2,Time[i],ema1)) continue;
...
void ProcessStartupSignal(DailyProtectionState &dailyState) {
   if(StartupSignalProcessed) return;
   if(Bars < SSLPeriod + 20) return;
   ...
   if(buySignal) {
      ...
      if(EnableTrading && !IsDailyTradingStopped(dailyState) && GetTotalEAOrders() < MaxOpenOrders && PassesEMAFilter(OP_BUY)) OpenBuy();
      return;
   }
   if(sellSignal) {
      DrawLiveSignal(0, false);
      if(EnableTrading && !IsDailyTradingStopped(dailyState) && GetTotalEAOrders() < MaxOpenOrders && PassesEMAFilter(OP_SELL)) OpenSell();
      return;
   }
}
...
int OnInit() {
   for(int initIndex = OrdersTotal() - 1; initIndex >= 0; initIndex--) {
      if(!OrderSelect(initIndex, SELECT_BY_POS, MODE_TRADES)) continue;
...
bool IsOneCandleOrderAllowed() {
   ...
   if(OrderCreatedThisCandle) {
      if(DayProfitLadderResumeTradeAttempt) return true;
      return false;
   }
   return true;
}
...
double GetEAFloatingPL() {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      ...
      int type = OrderType();
      if(type==OP_BUY || type==OP_SELL) floatingPL += OrderProfit() + OrderSwap() + OrderCommission();
   }
   return floatingPL;
}
...
void OnTickCore() {
   ...
   if(DayProfitLadderTargetCleanupPending && !DayProfitLadderTradingStopped) {
      if(!CloseAllEAOrdersForLadderReset()) { UpdateDashboardsThrottled(dailyState); return; }
   ...
   if(DayProfitLadderResumePending && DayProfitLadderResumeDirection != 0 && !DayProfitLadderTradingStopped) {
      ...
      if(resumeDirection == 1) ladderPendingCreated=CreateDayProfitLadderPending(OP_BUY);
      else if(resumeDirection == -1) ladderPendingCreated=CreateDayProfitLadderPending(OP_SELL);
   ...
   if(TradeOperationFailedThisTick) { UpdateDashboardsThrottled(dailyState); return; }
   if(EquityResetReEntryPending) ProcessEquityResetReEntry(dailyState);
   if(TradeOperationFailedThisTick) { UpdateDashboardsThrottled(dailyState); return; }

   if(ShowSSLLines) UpdateSSLChannelOnTick();
   UpdateEMALineOnChart();

   if(!TradeResetThisTick) ManageProfitReEntryMomentumGate();
   if(TradeOperationFailedThisTick) { UpdateDashboardsThrottled(dailyState); return; }
   if(!TradeResetThisTick) ProcessPendingReEntry(dailyState);
   if(TradeOperationFailedThisTick) { UpdateDashboardsThrottled(dailyState); return; }
   if(Bars >= SSLPeriod + 20 && !TradeResetThisTick) CheckForProfitableClosedOrder(dailyState);
   if(TradeOperationFailedThisTick) { UpdateDashboardsThrottled(dailyState); return; }
...
bool HasMinimumSameOrderGap(int orderType) {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      if(MathAbs(currentPrice - OrderOpenPrice()) < MinimumSameOrderGapRaw) return false;
   }
   return true;
}
...
double GetOppositeOrdersLots(int orderType) {
   ...
   for(int i = OrdersTotal()-1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber || OrderType() != oppositeType) continue;
      totalLots += OrderLots();
   }
...
bool CanOpenRiskOrder(int orderType, double requestedLot) {
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      if(MathAbs(existingLot - requestedLot) > 0.000001) continue;
      double pl = OrderProfit() + OrderSwap() + OrderCommission();
      double lossLimit = 3.0 * (requestedLot / 0.01);
      if(pl <= -lossLimit) return false;
   }
   return true;
}
...
bool HasExistingProfitReEntryOrder() {
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      if(StringFind(OrderComment(),"SSL Profit ReEntry",0)==0) return true;
   }
   return false;
}
...
bool IsRetryableTradeError(int err) {
   switch(err) { case 4: case 6: case 128: case 135: case 136: case 137: case 138: case 146: return true; }
   return false;
}
...
double GetRequiredStopDistance() {
   ...
   if(EnableFreezeLevelProtection) freezePts = MarketInfo(Symbol(),MODE_FREEZELEVEL);
...
bool IsDirectionBlockedAfterSL(int orderType) {
   if(ContinueTradingAfterSL || !EnableSLProtection) return false;
...
bool IsSafeToCreateMarketOrder(int orderType) {
   if(!IsDayProfitLadderTradingAllowed()) return false;
   if(!EnableSLProtection) return true;
   if(!IsConnected()) return false;
   if(HasBasketNewOrderLossLimit()) return false;
   if(MaxSameDirectionOrders>0 && CountDirectionOrders(orderType)>=MaxSameDirectionOrders) return false;
   if(IsDirectionBlockedAfterSL(orderType)) return false;
   return true;
}
...
bool IsTradeEnvironmentHealthy() {
   if(!IsConnected()) return false;
   RefreshRates();
   if(Bid<=0 || Ask<=0 || !IsTradeAllowed()) return false;
   return true;
}
...
void ResetRuntimeAfterServerError(DailyProtectionState &state) {
   ...
   LastLiveSignalCandle=Time[0];
   if(HasExistingProfitReEntryOrder()) LoadReEntryCounter();
...
bool ProcessServerRecovery(DailyProtectionState &state) {
   if(!ServerRecoveryPending) return false;
...
int FindExistingOrderForRequest(int orderType,double lots,double price,string orderComment,datetime requestTime) {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber || OrderType()!=orderType) continue;
      if(MathAbs(OrderLots()-lots)>0.0000001 || OrderComment()!=orderComment || OrderOpenTime()+3<requestTime) continue;
      if(orderType==OP_BUY || orderType==OP_SELL) {
         if(MathAbs(OrderOpenPrice()-price)<=priceTolerance) return OrderTicket();
      } else {
         if(MathAbs(OrderOpenPrice()-price)<=MathMax(Point,priceTolerance)) return OrderTicket();
      }
   }
   return -1;
}
...
bool CanSendTradeRequest(string operation,string details="") {
   if(MaxTradeRequestsPerTick<=0) return true;
   if(TradeRequestsThisTick>=MaxTradeRequestsPerTick) return false;
...
void UpdateDashboardsThrottled(DailyProtectionState &state,bool force=false) {
   uint now=GetTickCount();
   if(!force && LastDashboardUpdateMs!=0 && (uint)(now-LastDashboardUpdateMs)<(uint)MathMax(0,DashboardUpdateIntervalMs)) return;
   LastDashboardUpdateMs=now;
   if(ShowDashboard) UpdateDashboard(state);
   if(ShowLeftLiveOrdersDashboard) UpdateLeftLiveOrdersDashboard();
}

void LogTradeTiming(string operation,uint startedMs) {
   if(!EnableTradeTimingLog) return;
   uint elapsed=GetTickCount()-startedMs;
   if((int)elapsed>=SlowTradeRequestLogThresholdMs) Print("SLOW TRADE REQUEST: ",operation," ElapsedMs=",elapsed);
}

void OnTickPerformanceEnd(uint startedMs) {
   if(!EnableTickPerformanceLog) return;
   uint elapsed=GetTickCount()-startedMs;
   if((int)elapsed>=SlowTickLogThresholdMs) Print("SLOW EA TICK: ElapsedMs=",elapsed);
}
...
bool IsTradeErrorBlockedThisTick(string key) {
   for(int i=0; i<TradeErrorBlockedCount; i++) if(TradeErrorBlockedKeys[i]==key) return true;
   return false;
}
void BlockTradeErrorUntilNextTick(string key) {
   if(IsTradeErrorBlockedThisTick(key)) return;
   if(TradeErrorBlockedCount<200) { TradeErrorBlockedKeys[TradeErrorBlockedCount]=key; TradeErrorBlockedCount++; }
}
...
bool Passes30MinuteMomentumFilter(int orderType) {
   if(!Enable30MinuteMomentumFilter) return true;
   ...
   if(orderType == OP_BUY) return (diff > threshold);
   return (diff < -threshold);
}

int DeferredDirection(int orderType) {
   if(orderType==OP_BUY || orderType==OP_BUYSTOP || orderType==OP_BUYLIMIT) return OP_BUY;
   if(orderType==OP_SELL || orderType==OP_SELLSTOP || orderType==OP_SELLLIMIT) return OP_SELL;
   return -1;
}

bool PassesDeferredMomentum(int orderType) {
   if(!Enable30MinuteMomentumFilter || !Enable30MinuteMomentumForAllOrders) return true;
   int direction=DeferredDirection(orderType);
   if(direction!=OP_BUY && direction!=OP_SELL) return true;
...
int FindDeferredOrder(string symbol,int orderType,string comment,int magic) {
   for(int i=0; i<MAX_DEFERRED_ORDERS; i++) {
      if(!DeferredActive[i]) continue;
      if(DeferredSymbol[i]!=symbol || DeferredType[i]!=orderType || DeferredMagic[i]!=magic) continue;
      if(DeferredComment[i]==comment) return i;
   }
   return -1;
}
...
int QueueDeferredOrder(string symbol,int orderType,double lots,double price,int slippage,double stopLoss,double takeProfit,string comment,int magic,color arrowColor,bool bypassDeferred=false) {
   ...
   for(int i=0; i<MAX_DEFERRED_ORDERS; i++) {
      ...
      DeferredSymbol[i]=symbol; DeferredType[i]=orderType; DeferredLots[i]=lots;
      DeferredPrice[i]=price; DeferredSlippage[i]=slippage; DeferredSL[i]=stopLoss;
      DeferredTP[i]=takeProfit; DeferredComment[i]=comment; DeferredMagic[i]=magic;
      DeferredColor[i]=arrowColor; DeferredCreated[i]=TimeCurrent();
      return -2;
   }
   return -1;
}
...
void ProcessDeferredOrders() {
   if(!EnableTrading || !IsDayProfitLadderTradingAllowed()) return;
   for(int i=0; i<MAX_DEFERRED_ORDERS; i++) {
      if(!DeferredActive[i]) continue;
      int type=DeferredType[i];
      if(Enable30MinuteMomentumFilter && Enable30MinuteMomentumForAllOrders && !PassesDeferredMomentum(type)) continue;
      if(GetTotalEAOrders() >= MaxOpenOrders) continue;
      double sendPrice=DeferredPrice[i];
      if(type==OP_BUYSTOP || type==OP_SELLSTOP || type==OP_BUYLIMIT || type==OP_SELLLIMIT) {
         RefreshRates();
         double minGap=GetRequiredStopDistance();
         if(type==OP_BUYSTOP) sendPrice=MathMax(sendPrice,Ask+minGap);
         else if(type==OP_SELLSTOP) sendPrice=MathMin(sendPrice,Bid-minGap);
      ...
      if(ticket>0) {
         if(type==OP_BUY || type==OP_SELL) { OrderCreatedThisCandle=true; LastOrderCandleTime=Time[0]; }
         if(StringFind(DeferredComment[i],"SSL Profit ReEntry",0)==0) { reEntryCounter++; SaveReEntryCounter(); ReEntryRetryPending=false; }
         DeferredActive[i]=false; InvalidateTotalEAOrdersCache();
      }
   }
}

double CalculateDefaultProfitTargetPrice(int orderType,double openPrice,double orderLots,double profitUSD) {
   if(openPrice<=0.0 || orderLots<=0.0 || profitUSD<=0.0) return 0.0;
   ...
   if(priceDistance<=0.0) return 0.0;
   double target=0.0;
   if(orderType==OP_BUY || orderType==OP_BUYLIMIT || orderType==OP_BUYSTOP) target=openPrice+priceDistance;
   else if(orderType==OP_SELL || orderType==OP_SELLLIMIT || orderType==OP_SELLSTOP) target=openPrice-priceDistance;
   else return 0.0;
...
void RegisterPostOrderSLTPVerification(int ticket,double expectedSL,double expectedTP) {
   if(!EnablePostOrderSLTPVerification || ticket<=0) return;
   for(int i=0; i<MAX_POST_ORDER_SLTP_VERIFY; i++) if(PostOrderSLTPVerifyActive[i] && PostOrderSLTPVerifyTicket[i]==ticket) return;
   int slot=-1;
   for(int i=0; i<MAX_POST_ORDER_SLTP_VERIFY; i++) if(!PostOrderSLTPVerifyActive[i]) { slot=i; break; }
   if(slot<0) return;
   double openPrice=0.0; int orderType=-1; double lots=0.0;
   if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) {
      openPrice=OrderOpenPrice(); orderType=OrderType(); lots=OrderLots();
      if(expectedSL<=0.0) {
         if(orderType==OP_BUY) expectedSL=openPrice-Slippage-(StopLossUSD*100);
         else if(orderType==OP_SELL) expectedSL=openPrice+Slippage+(StopLossUSD*100);
      }
      if(expectedTP<=0.0) {
         if(orderType==OP_BUY) expectedTP=openPrice+Slippage+(DefaultOrderProfitUSD*100);
         else if(orderType==OP_SELL) expectedTP=openPrice-Slippage-(DefaultOrderProfitUSD*100);
      }
   }
   PostOrderSLTPVerifyActive[slot]=true; PostOrderSLTPVerifyTicket[slot]=ticket; PostOrderSLTPVerifyCreated[slot]=TimeCurrent();
   PostOrderSLTPVerifyChecked[slot]=false; PostOrderSLTPVerifyStatus[slot]="CHECKING"; PostOrderSLTPVerifyExpectedSL[slot]=expectedSL;
   PostOrderSLTPVerifyExpectedTP[slot]=expectedTP; PostOrderSLTPVerifyExpectedOpen[slot]=openPrice;
   PostOrderSLTPVerifyType[slot]=orderType; PostOrderSLTPVerifyLots[slot]=lots;
}

bool PostOrderSLTPWithinTolerance(int orderType,double openPrice,double expectedPrice,double actualPrice,bool isSL) {
   if(expectedPrice<=0.0) return (actualPrice<=0.0);
   if(actualPrice<=0.0 || openPrice<=0.0) return false;
   double expectedDistance=MathAbs(expectedPrice-openPrice);
   double actualDistance=MathAbs(actualPrice-openPrice);
   if(expectedDistance<=Point) return (MathAbs(actualPrice-expectedPrice)<=Point);
   double tolerance=expectedDistance*(PostOrderSLTPTolerancePercent/100.0);
   if(MathAbs(actualDistance-expectedDistance)>tolerance) return false;
   if(orderType==OP_BUY || orderType==OP_BUYSTOP || orderType==OP_BUYLIMIT) {
      if(isSL && actualPrice>=openPrice) return false;
      if(!isSL && actualPrice<=openPrice) return false;
...
string GetPostOrderSLTPVerificationStatus(int ticket) {
   if(ticket<=0) return "NOT CHECKED";
   for(int i=0; i<MAX_POST_ORDER_SLTP_VERIFY; i++) if(PostOrderSLTPVerifyTicket[i]==ticket) {
      if(PostOrderSLTPVerifyStatus[i]!="") return PostOrderSLTPVerifyStatus[i];
      if(PostOrderSLTPVerifyActive[i] && !PostOrderSLTPVerifyChecked[i]) return "CHECKING";
      if(PostOrderSLTPVerifyChecked[i]) return "CHECKED";
   }
   return "NOT CHECKED";
}

void ProcessPostOrderSLTPVerification() {
   if(!EnablePostOrderSLTPVerification) return;
   datetime now=TimeCurrent();
   for(int i=0; i<MAX_POST_ORDER_SLTP_VERIFY; i++) {
      if(!PostOrderSLTPVerifyActive[i] || PostOrderSLTPVerifyChecked[i]) continue;
      if((now-PostOrderSLTPVerifyCreated[i]) < PostOrderSLTPVerificationDelaySeconds) continue;
      int ticket=PostOrderSLTPVerifyTicket[i];
      PostOrderSLTPVerifyChecked[i]=true; PostOrderSLTPVerifyActive[i]=false;
      if(!OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) { PostOrderSLTPVerifyStatus[i]="CHECKED"; continue; }
      int orderType=OrderType(); double openPrice=OrderOpenPrice(); double actualSL=OrderStopLoss(); double actualTP=OrderTakeProfit();
      double expectedSL=PostOrderSLTPVerifyExpectedSL[i]; double expectedTP=PostOrderSLTPVerifyExpectedTP[i];
      double expectedOpen=PostOrderSLTPVerifyExpectedOpen[i]; double actualPending=openPrice;  
      ...
      if(isPendingOrder) pendingPriceOK=(MathAbs(actualPending-expectedOpen) <= Point*50);
      ...
      if(isPendingOrder) allOK=allOK && pendingPriceOK;
      if(allOK) { PostOrderSLTPVerifyStatus[i]="VERIFIED"; continue; }
      if(!CanSendTradeRequest("PostOrderSLTPVerify","Ticket="+IntegerToString(ticket))) { PostOrderSLTPVerifyStatus[i]="FAILED"; continue; }
      RefreshRates(); ResetLastError();
      if(OrderModify(ticket,openPrice,expectedSL,expectedTP,OrderExpiration(),CLR_NONE)) PostOrderSLTPVerifyStatus[i]="VERIFIED";
      else PostOrderSLTPVerifyStatus[i]="FAILED";
   }
}

string GetOrderTypeText(int orderType) {
   switch(orderType) { case OP_BUY: return "BUY"; case OP_SELL: return "SELL"; case OP_BUYSTOP: return "BUY STOP"; case OP_SELLSTOP: return "SELL STOP"; case OP_BUYLIMIT: return "BUY LIMIT"; case OP_SELLLIMIT: return "SELL LIMIT"; default: return "UNKNOWN"; }
}
...
bool IsTradingSloweHours() {
   ...
   string hours[]; int count = StringSplit(DubaiTradingPauseHours, ',', hours);
   if(count <= 0) return false;
   for(int i=0; i<count; i++) {
      int pauseHour = (int)StringToInteger(hours[i]);
      if(pauseHour == 24) pauseHour = 0;
      if(currentHour == pauseHour) return true;
   }
   return false;
}
...
int SafeOrderSend(string symbol,int orderType,double lots,double price,int slippage,double stopLoss,double takeProfit,string comment,int magic,color arrowColor) {
   ...
   if(takeProfit<=0.0 && DefaultOrderProfitUSD>0.0) {
      ...
      if(defaultTP>0.0) takeProfit=defaultTP;
   }
   double safeSL=stopLoss;
   if(safeSL>0.0 && !PrepareStopLossForOrder(orderType,sendPrice,safeSL)) {
      BlockTradeErrorUntilNextTick(key); MarkServerError(130,"OrderSend/unsafe-SL"); return -1;
   }
   datetime requestTime=TimeCurrent();
   if(!CanSendTradeRequest("OrderSend",comment)) return -1;
   ResetLastError(); uint tradeStartMs=GetTickCount();
   ...
   if(existing>0) {
      if(OrderSelect(existing,SELECT_BY_TICKET,MODE_TRADES)) RegisterPostOrderSLTPVerification(existing,safeSL,takeProfit);
      ServerRecoveryPending=false; return existing;
   }
   BlockTradeErrorUntilNextTick(key); MarkServerError(err,"OrderSend"); return -1;
}
...
bool SafeOrderClose(int ticket,double lots,int orderType,int slippage,color arrowColor) {
   ...
   if(IsTradeErrorBlockedThisTick(key)) return false;
   if(!OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) { if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_HISTORY)) return true; return false; }
   RefreshRates();
   double closePrice=(OrderType()==OP_BUY)?Bid:Ask; closePrice=NormalizeDouble(closePrice,Digits);
   if(!CanSendTradeRequest("OrderClose","Ticket="+IntegerToString(ticket))) return false;
   ResetLastError(); bool result=OrderClose(ticket,OrderLots(),closePrice,slippage,arrowColor);
   if(result) { InvalidateTotalEAOrdersCache(); return true; }
   int err=GetLastError();
   if(!OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) { if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_HISTORY)) return true; }
   BlockTradeErrorUntilNextTick(key); MarkServerError(err,"OrderClose"); return false;
}
...
bool SafeOrderModify(int ticket,double openPrice,double stopLoss,double takeProfit,datetime expiration,color arrowColor) {
   ...
   int orderType=OrderType(); RefreshRates(); double requestedSL=stopLoss;
   if(requestedSL>0.0) {
      if(!PrepareStopLossForOrder(orderType,OrderOpenPrice(),requestedSL)) { BlockTradeErrorUntilNextTick(key); MarkServerError(130,"OrderModify/unsafe-SL"); return false; }
      if(orderType==OP_BUY && OrderStopLoss()>0.0 && requestedSL<=OrderStopLoss()) { BlockTradeErrorUntilNextTick(key); return false; }
      if(orderType==OP_SELL && OrderStopLoss()>0.0 && requestedSL>=OrderStopLoss()) { BlockTradeErrorUntilNextTick(key); return false; }
   }
   ...
   ResetLastError();
   if(!CanSendTradeRequest("OrderModify","Ticket="+IntegerToString(ticket))) return false;
   bool modified=OrderModify(ticket,openPrice,requestedSL,takeProfit,expiration,arrowColor);
   if(modified) return true;
   int err=GetLastError(); BlockTradeErrorUntilNextTick(key); MarkServerError(err,"OrderModify"); return false;
}
...
bool SafeOrderDelete(int ticket,color arrowColor) {
   ...
   if(ageSeconds < requiredAgeSeconds) return false;
   if(!CanSendTradeRequest("OrderDelete","Ticket="+IntegerToString(ticket))) return false;
   ResetLastError(); bool result=OrderDelete(ticket,arrowColor);
   if(result) return true;
   int err=GetLastError();
   if(!OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) return true;
   BlockTradeErrorUntilNextTick(key); MarkServerError(err,"OrderDelete"); return false;
}

int GetM5Direction() {
   int candles = M5ConfirmationCandles;
   if(candles < 1) candles = 1; if(candles > 5) candles = 5;
   int bullish = 0, bearish = 0;
   for(int shift=1; shift<=candles; shift++) {
      double m5Open  = iOpen(Symbol(), PERIOD_M5, shift); double m5Close = iClose(Symbol(), PERIOD_M5, shift);
      if(m5Open <= 0.0 || m5Close <= 0.0) continue;
      if(m5Close > m5Open) bullish++; else if(m5Close < m5Open) bearish++;
   }
   if(bullish > bearish && bullish >= (candles/2 + 1)) return 1;
   if(bearish > bullish && bearish >= (candles/2 + 1)) return -1;
   return 0;
}

double GetM5CandleATRRatio() {
   if(!EnableATRVolatilityProtection) return 0.0;
   int tf = ATRVolatilityTimeframe; if(tf <= 0) tf = PERIOD_M5;
   int period = ATRVolatilityPeriod; if(period < 2) period = 2;
   double high1 = iHigh(Symbol(), tf, 1); double low1  = iLow(Symbol(), tf, 1); double atr1  = iATR(Symbol(), tf, period, 1);
   if(high1 <= 0.0 || low1 <= 0.0 || atr1 <= 0.0) return 0.0;
   return (high1-low1)/atr1;
}

bool IsExtremeVolatility() {
   if(!EnableATRVolatilityProtection) return false;
...
double GetMarketMomentLot(int orderType) {
   if(orderType != OP_BUY && orderType != OP_SELL) return NormalizeLots(OriginalLots);
   int sslDirection = GetCurrentSSLDirection();
   int requestedDirection = (orderType == OP_BUY) ? 1 : -1;
   if(sslDirection != requestedDirection) return NormalizeLots(0.01);
   int h1Direction = GetH1Direction(); bool h1Aligned = (h1Direction == requestedDirection); bool h1Opposite = (h1Direction == -requestedDirection);
   int m5Direction = GetM5Direction(); bool m5Aligned = (m5Direction == requestedDirection);
   int emaShift = InpEMAPriceShift; if(emaShift < 0) emaShift = 0;
   int chartBars = iBars(Symbol(), Period()); if(chartBars <= 0) chartBars = Bars; if(emaShift >= chartBars) emaShift = chartBars - 1;
   double ema = iMA(Symbol(), Period(), InpEMA200Period, 0, MODE_EMA, PRICE_CLOSE, emaShift);
   RefreshRates(); double emaPrice = (emaShift == 0) ? ((orderType == OP_BUY) ? Ask : Bid) : iClose(Symbol(), Period(), emaShift);
   int emaDirection = 0; if(ema > 0.0 && emaPrice > 0.0) { if(emaPrice > ema) emaDirection = 1; else if(emaPrice < ema) emaDirection = -1; }
   bool emaAligned = (emaDirection == requestedDirection);
   double momentumDiff = Get30MinDifference(orderType); double momentumThreshold = MathAbs(Min30MinutePriceDifference); bool momentumAligned = false;
...
void CheckRecoveryOrders() {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderMagicNumber() != MagicNumber || OrderSymbol() != Symbol()) continue;
      int parentTicket = OrderTicket(); int parentType   = OrderType();
      if(parentType != OP_BUY && parentType != OP_SELL) continue;
      string comment = OrderComment();
      if(StringFind(comment, "RECOVERY_") == 0) continue;
      bool validParent = false;
      if(StringFind(comment, "SSL Long") >= 0 || StringFind(comment, "SSL Short") >= 0 || StringFind(comment, "ReEntry") >= 0) validParent = true;
      if(!validParent) continue;
      if(HasRecoveryOrder(parentTicket)) continue;
      ...
      if(profit > recoveryTrigger) continue;
      if(parentType == OP_BUY) { if(!IsBuySignal(0)) continue; } else { if(!IsSellSignal(0)) continue; }
      ...
      if(recoveryTicket > 0) { OrderCreatedThisCandle = true; LastOrderCandleTime    = Time[0]; }
      break;
   }
}
...
bool HasRecoveryOrder(int ParentTicket) {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      if(OrderComment()==txt) return true;
   }
   return false;
}

void ManageRecoveryBasket() {
   if(!EnableRecoveryOrders) return;
   for(int i = OrdersTotal()-1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      string comment = OrderComment();
      ...
      if(!OrderSelect(parentTicket,SELECT_BY_TICKET)) continue;
...
double GetDayProfitLadderTargetPercent(int stage) {
   if(stage <= 0 || DayProfitLadderStartBalance <= 0.0) return DayProfitLadderStartBalance;
   double step = MathAbs(DayProfitLadder1Percent) / 100.0;
   if(step <= 0.0) return DayProfitLadderStartBalance;
   return DayProfitLadderStartBalance * (1.0 + step * stage);
}
double GetDayProfitLadderTarget(int stage) {
   if(DayProfitLadderStartBalance <= 0.0) return 0.0;
   if(stage <= 0) return DayProfitLadderStartBalance;
   double stepAmount = MathAbs(DayProfitLadder1Amount);
   if(stepAmount <= 0.0) return DayProfitLadderStartBalance;
   return DayProfitLadderStartBalance + (stepAmount * stage);
}
double GetDayProfitLadderProtection(int stage) {
   if(DayProfitLadderStartBalance<=0.0) return 0.0;
   if(stage<=0) return DayProfitLadderStartBalance * (1.0-DayProfitInitialProtectionPercent/100.0);
   double step=MathAbs(DayProfitLadder1Percent)/100.0;
...
void InitializeDayProfitLadder() {
   ...
   DayProfitLadderStage = 0; DayProfitLadderTradingStopped = false; DayProfitLadderTargetReachedCandle = 0;
   DayProfitLadderResumePending = false; DayProfitLadderResumeDirection = 0; DayProfitLadderResumeTradeAttempt = false;
   DayProfitLadderTargetCleanupPending = false; DayProfitLadderNextTargetEquity = GetDayProfitLadderTarget(1);
   DayProfitLadderProtectionEquity = DayProfitLadderStartBalance * (1.0 - DayProfitInitialProtectionPercent / 100.0);
   DayProfitLadderInitialized = true; SaveDayProfitLadderState();
}

void LoadDayProfitLadderState() { InitializeDayProfitLadder(); }
void CloseAndDeleteAllEAOrdersOnTradingStop() {
   int freshPendingTickets[1000]; int freshPendingCount = 0;
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      int type=OrderType();
      if(type==OP_BUYSTOP || type==OP_SELLSTOP || type==OP_BUYLIMIT || type==OP_SELLLIMIT) {
         if(freshPendingCount < 1000) freshPendingTickets[freshPendingCount++]=OrderTicket();
      }
   }
   int freshOpenTickets[1000]; int freshOpenCount=0;
   for(int j=OrdersTotal()-1; j>=0; j--) {
      if(!OrderSelect(j,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      int marketType=OrderType();
      if(marketType==OP_BUY || marketType==OP_SELL) {
         if(freshOpenCount < 1000) freshOpenTickets[freshOpenCount++]=OrderTicket();
      }
   }
   for(int k=0; k<freshOpenCount; k++) {
      ...
      double lots=OrderLots(); SafeOrderClose(ticket,lots,type,Slippage,(type==OP_BUY ? clrRed : clrBlue));
   }
   for(int p=0; p<freshPendingCount; p++) {
      int pendingTicket=freshPendingTickets[p];
      if(!OrderSelect(pendingTicket,SELECT_BY_TICKET,MODE_TRADES)) continue; 
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      int pendingType=OrderType();
      if(pendingType!=OP_BUYSTOP && pendingType!=OP_SELLSTOP && pendingType!=OP_BUYLIMIT && pendingType!=OP_SELLLIMIT) continue; 
      SafeOrderDelete(pendingTicket,clrRed);
   }
}

bool CloseAllEAOrdersForLadderReset() {
   bool anyRemaining = false; int marketTickets[1000]; int marketCount = 0; int pendingTickets[1000]; int pendingCount = 0;
   if(DayProfitLadderCleanupStartTime > 0) {
      if((int)(TimeCurrent() - DayProfitLadderCleanupStartTime) >= LADDER_CLEANUP_TIMEOUT_SECONDS) {
         DayProfitLadderCleanupStartTime = 0; return true; 
      }
   } else DayProfitLadderCleanupStartTime = TimeCurrent();

   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      int type=OrderType();
      if(type==OP_BUY || type==OP_SELL) { if(marketCount<1000) marketTickets[marketCount++]=OrderTicket(); }
      else if(type==OP_BUYSTOP || type==OP_SELLSTOP || type==OP_BUYLIMIT || type==OP_SELLLIMIT) { if(pendingCount<1000) pendingTickets[pendingCount++]=OrderTicket(); }
   }
   for(int m=0; m<marketCount; m++) {
      ...
      if(!SafeOrderClose(ticket,lots,type,Slippage,(type==OP_BUY ? clrRed : clrBlue))) anyRemaining=true;
   }
   for(int q=0; q<pendingCount; q++) {
      ...
      if(!SafeOrderDelete(ticket,clrRed)) anyRemaining=true;
   }
   for(int j=OrdersTotal()-1; j>=0; j--) {
      if(!OrderSelect(j,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()==Symbol() && OrderMagicNumber()==MagicNumber) { anyRemaining=true; break; }
   }
...
bool CreateDayProfitLadderPending(int direction) {
   if(!EnableTrading || DayProfitLadderTradingStopped) return false;
   if(direction!=OP_BUY && direction!=OP_SELL) return false;
   if(ServerRecoveryPending || !IsConnected() || GetTotalEAOrders() >= MaxOpenOrders) return false;
   RefreshRates();
   int pendingType=(direction==OP_BUY) ? OP_BUYSTOP : OP_SELLSTOP;
   double minimumGap=GetRequiredStopDistance();
   double gap=MathAbs(ProfitReEntryGapRaw); if(gap<minimumGap) gap=minimumGap;
   double entryPrice=0.0;
   if(direction==OP_BUY) entryPrice=Ask+gap; else entryPrice=Bid-gap;
   entryPrice=NormalizeDouble(entryPrice,Digits);
   reEntryCounter=0; SaveReEntryCounter();
   ChangeLots(0.0, direction==OP_BUY ? "Day Ladder Buy Stop" : "Day Ladder Sell Stop", direction, 0);
   Lots=NormalizeLots(Lots); if(Lots<=0.0) return false;
   double slDistance=CalculatePriceDistanceUSD(StopLossUSD,Lots); if(slDistance<=0.0) return false;
   ...
   InvalidateTotalEAOrdersCache(); return true;
}

void ManageDayProfitLadder() {
   if(!EnableDayProfitLadder) return;
   datetime today = GetDayProfitLadderDate();
   if(!DayProfitLadderInitialized || DayProfitLadderDate != today) { InitializeDayProfitLadder(); return; }
   if(DayProfitLadderTradingStopped) { CloseAndDeleteAllEAOrdersOnTradingStop(); return; }
   if(DayProfitLadderStartBalance <= 0.0) return;
   ...
   if(step > 0.0 && equity >= DayProfitLadderStartBalance * (1.0 + step)) {
      ...
      if(reachedStage > DayProfitLadderStage) {
         ...
         if(DayProfitLadderResumeDirection == 0) DayProfitLadderResumeDirection = LastLiveSSLDirection;
   ...
   if(equity <= DayProfitLadderProtectionEquity) {
      DayProfitLadderTradingStopped = true; DayProfitLadderTargetReachedCandle = 0; DayProfitLadderResumePending = false;
      DayProfitLadderResumeDirection = 0; DayProfitLadderResumeTradeAttempt = false; DayProfitLadderTargetCleanupPending = false;
      SaveDayProfitLadderState(); CloseAndDeleteAllEAOrdersOnTradingStop();
      for(int dpl_i=0; dpl_i<MAX_DEFERRED_ORDERS; dpl_i++) DeferredActive[dpl_i]=false;
      EquityResetReEntryPending=false; ReEntryRetryPending=false;
   }
}

bool IsDayProfitLadderTradingAllowed() {
   if(!EnableDayProfitLadder) return true;
   ManageDayProfitLadder();
   if(DayProfitLadderTradingStopped) return false;
   return true;
}

void InitializeDailyProtectionState(DailyProtectionState &state) {
   ManageDayProfitLadder();
   state.DayDate = DayProfitLadderDate; state.DayStartBalance = DayProfitLadderStartBalance;
   state.DayProtectedBalance = DayProfitLadderProtectionEquity; state.ClosedOrdersToday = CountClosedOrdersSinceInitialization();
   state.TradingStopped = DayProfitLadderTradingStopped; state.Initialized = true;
   DailyProtectionStartTime = TimeCurrent();
}

void StartProtectedEquityWait() {
   ProtectedEquityWaitActive = true; ProtectedEquityWaitStartTime = TimeCurrent(); EquityResetReEntryPending = false;
}
...
bool IsProtectedEquityWaiting() {
   ...
   ProtectedEquityWaitActive = false; return false;
}
...
void QueueEquityResetReEntry() {
   EquityResetReEntryPending = true; OrderCreatedThisCandle = false; LastOrderCandleTime = 0;
}

void ProcessEquityResetReEntry(DailyProtectionState &state) {
   if(!EquityResetReEntryPending || !EnableTrading || IsDailyTradingStopped(state) || Bars < SSLPeriod + 20) return;
   if(GetTotalEAOrders() > 0) { EquityResetReEntryPending = false; return; }
   if(GetTotalEAOrders() >= MaxOpenOrders) return;
   int currentDirection = GetCurrentSSLDirection();
   if(currentDirection > 0) {
      OrderCreatedThisCandle = false; LastOrderCandleTime = 0; DrawLiveSignal(0, true);
      int beforeOrders = GetTotalEAOrders(); OpenBuy(); int afterOrders = GetTotalEAOrders();
      if(afterOrders > beforeOrders) { EquityResetReEntryPending = false; TradeResetThisTick = true; }
      return;
   }
   if(currentDirection < 0) {
      OrderCreatedThisCandle = false; LastOrderCandleTime = 0; DrawLiveSignal(0, false);
      int beforeOrders = GetTotalEAOrders(); OpenSell(); int afterOrders = GetTotalEAOrders();
      if(afterOrders > beforeOrders) { EquityResetReEntryPending = false; TradeResetThisTick = true; }
      return;
   }
}
...
void UpdateDailyLossProtection(DailyProtectionState &state) {
   if(!EnableDayProfitLadder) return;
   state.DayDate = DayProfitLadderDate; state.DayStartBalance = DayProfitLadderStartBalance;
   state.DayProtectedBalance = DayProfitLadderProtectionEquity; state.TradingStopped = DayProfitLadderTradingStopped;
   state.ClosedOrdersToday = CountClosedOrdersSinceInitialization();
}
...
void DeleteOppositePendingOrders(int newSignalType) {
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      if(deleteOrder) SafeOrderDelete(OrderTicket(), clrYellow);
   }
}
...
void CloseOppositeOrders(int newSignalType) {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      int orderType = OrderType(); int ticket = OrderTicket(); double lots = OrderLots();
      double orderPL = OrderProfit() + OrderSwap() + OrderCommission();
      if(orderPL > closeOppositeLossThreshold) continue;
...
void UpdateSSLChannelOnTick() {
   ...
   for(int i = maxRecentBars; i >= 0; i--) {
      ...
      double up1, down1, up2, down2; int hlv1, hlv2;
      CalculateSSL(i, up1, down1, hlv1); CalculateSSL(i + 1, up2, down2, hlv2);
...
void InitializeLastProcessedClosedOrder() {
   datetime latestCloseTime = 0; int latestTicket = -1;
   for(int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      if(OrderType() != OP_BUY && OrderType() != OP_SELL) continue;
      if(OrderCloseTime() > latestCloseTime) {
         latestCloseTime = OrderCloseTime(); latestTicket = OrderTicket();
      }
   }
   LastProcessedClosedOrderTime = latestCloseTime; LastProcessedClosedTicket = latestTicket;
}

void CheckForProfitableClosedOrder(DailyProtectionState &state) {
   datetime latestCloseTime = 0; double latestProfit = 0; int latestTicket = -1, latestType = -1; double latestClosePrice = 0;
   for(int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      if(OrderType() != OP_BUY && OrderType() != OP_SELL) continue;
      if(OrderCloseTime() <= latestCloseTime) continue;
      latestCloseTime = OrderCloseTime(); latestTicket = OrderTicket(); latestType = OrderType();
      latestProfit = OrderProfit() + OrderSwap() + OrderCommission(); latestClosePrice = OrderClosePrice();
   }
   if(latestTicket < 0) return;
   if(latestTicket == LastProcessedClosedTicket && latestCloseTime == LastProcessedClosedOrderTime) return;
   LastProcessedClosedTicket = latestTicket; LastProcessedClosedOrderTime = latestCloseTime;
...
bool HasPendingProfitReEntry(int pendingType) {
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber || OrderType()!=pendingType) continue;
      if(StringFind(OrderComment(),"SSL Profit ReEntry",0)==0) return true;
   }
   return false;
}
...
void CreateProfitReEntryStop(int closedOrderType, double closedPrice, DailyProtectionState &state, bool afterStopLoss=false) {
   ...
   if(ServerRecoveryPending || !IsConnected()) { ReEntryRetryPending=true; ReEntryRetryClosedType=closedOrderType; ReEntryRetryClosedPrice=closedPrice; return; }
   if(HasBasketNewOrderLossLimit() || IsDirectionBlockedAfterSL(closedOrderType) || GetTotalEAOrders() >= MaxOpenOrders || (MaxSameDirectionOrders > 0 && CountDirectionOrders(closedOrderType) >= MaxSameDirectionOrders)) return;
   RefreshRates();
   ...
   entryPrice = NormalizeDouble(entryPrice, Digits);
   ResetLastError();
   if(afterStopLoss) Lots = NormalizeLots(0.01);
   else {
      if(pendingType == OP_BUYSTOP) ChangeLots(GetOpenPL(OP_SELL), "SSL Profit ReEntry Buy Stop", OP_BUY,0);
      else ChangeLots(GetOpenPL(OP_BUY), "SSL Profit ReEntry Sell Stop", OP_SELL,0);
   }
   int requestedReEntryNumber = reEntryCounter + 1;
   double slDistance = CalculatePriceDistanceUSD(StopLossUSD, Lots); if(slDistance <= 0) return;
   double stopLoss = (pendingType == OP_BUYSTOP) ? (entryPrice - slDistance) : (entryPrice + slDistance); stopLoss = NormalizeDouble(stopLoss, Digits);
   int ticket = SafeOrderSend(Symbol(), pendingType, Lots, entryPrice, Slippage, stopLoss, 0, orderComment, MagicNumber, orderColor);
   if(ticket < 0) {
      if(ServerRecoveryPending) { ReEntryRetryPending=true; ReEntryRetryClosedType=closedOrderType; ReEntryRetryClosedPrice=closedPrice; }
   } else {
      reEntryCounter=requestedReEntryNumber; SaveReEntryCounter(); ReEntryRetryPending=false;
   }
}
...
int GetTotalEAOrders() {
   if(CachedTotalEAOrdersTick==CurrentTickSequence && CachedTotalEAOrders>=0) return CachedTotalEAOrders;
   int count = 0;
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      int type = OrderType();
      if(type==OP_BUY || type==OP_SELL || type==OP_BUYSTOP || type==OP_SELLSTOP || type==OP_BUYLIMIT || type==OP_SELLLIMIT) count++;
   }
   CachedTotalEAOrders=count; CachedTotalEAOrdersTick=CurrentTickSequence; return count;
}

void InvalidateTotalEAOrdersCache() { CachedTotalEAOrders=-1; CachedTotalEAOrdersTick=-1; }

void OpenBuy() {
   if(!IsSafeToCreateMarketOrder(OP_BUY) || !PassesEMAFilter(OP_BUY) || !IsOneCandleOrderAllowed() || GetTotalEAOrders() >= MaxOpenOrders || !HasMinimumSameOrderGap(OP_BUY)) return;
   reEntryCounter=0; SaveReEntryCounter(); ChangeLots(GetOpenPL(OP_SELL),"SSL Long",OP_BUY,0); RefreshRates();
   double slDistance = CalculatePriceDistanceUSD(StopLossUSD, Lots); if(slDistance <= 0) return;
   ...
   if(ticket > 0) { OrderCreatedThisCandle = true; LastOrderCandleTime = Time[0]; }
}
...
double NormalizeLots(double lots) {
   ...
   if(lotStep <= 0.0) lotStep = minLot > 0.0 ? minLot : 0.01;
   lots = MathMax(minLot, MathMin(maxLot, lots));
   lots = MathFloor((lots + 1e-9) / lotStep) * lotStep;
   lots = MathMax(minLot, MathMin(maxLot, lots));
   int digits = 0; double step = lotStep;
   while(digits < 8 && MathAbs(step - MathRound(step)) > 1e-8) { step *= 10.0; digits++; }
   return NormalizeDouble(lots, digits);
}

void OpenSell() {
   if(!IsSafeToCreateMarketOrder(OP_SELL) || !PassesEMAFilter(OP_SELL) || !IsOneCandleOrderAllowed() || GetTotalEAOrders() >= MaxOpenOrders || !HasMinimumSameOrderGap(OP_SELL)) return;
   reEntryCounter=0; SaveReEntryCounter(); ChangeLots(GetOpenPL(OP_BUY),"SSL Short",OP_SELL,0); RefreshRates();
   double slDistance = CalculatePriceDistanceUSD(StopLossUSD, Lots); if(slDistance <= 0) return;
...
void ManageProfitLadder() {
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      if(!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderSymbol() != Symbol() || OrderMagicNumber() != MagicNumber) continue;
      if(StringFind(OrderComment(), "RECOVERY_") == 0) continue;
      int orderType = OrderType();
      if(orderType != OP_BUY && orderType != OP_SELL) continue;
      ...
      if(EnableProfitLadder1 && ladder1Profit > 0 && currentProfit < ladder1StopMaxPrice) {
         ...
         if(ladder1Level >= 1) lockedProfit = ladder1Level * ladder1Profit;
      }
      if(EnableProfitLadder2 && ladder2Profit > 0 && currentProfit >= ladder1StopMaxPrice) {
         ...
         if(ladder2Level >= 1) lockedProfit = ladder2Level * ladder2Profit;
      }
      ...
      if(OrderStopLoss() > 0) {
         ...
         if(orderType == OP_BUY) existingPriceDistance = OrderStopLoss() - OrderOpenPrice();
         if(orderType == OP_SELL) existingPriceDistance = OrderOpenPrice() - OrderStopLoss();
      ...
      } else if(EnableProfitLadder2 && ladder2Profit>0.0) {
      ...
      if(nextProfitTargetUSD>0.0) {
         ...
         if(nextTP>0.0) {
            ...
            if(orderType==OP_BUY && nextTP-Ask<minimumTPDistance) validNextTP=false;
            if(orderType==OP_SELL && Bid-nextTP<minimumTPDistance) validNextTP=false;
            if(validNextTP) {
               if(orderType==OP_BUY) { if(desiredTakeProfit<=0.0 || nextTP>desiredTakeProfit+Point) desiredTakeProfit=nextTP; }
               else if(orderType==OP_SELL) { if(desiredTakeProfit<=0.0 || nextTP<desiredTakeProfit-Point) desiredTakeProfit=nextTP; }
            }
         }
      }

      bool takeProfitNeedsModify=false;
      if(orderType==OP_BUY) { if(desiredTakeProfit>0.0 && (OrderTakeProfit()<=0.0 || desiredTakeProfit>OrderTakeProfit()+Point)) takeProfitNeedsModify=true; }
      else if(orderType==OP_SELL) { if(desiredTakeProfit>0.0 && (OrderTakeProfit()<=0.0 || desiredTakeProfit<OrderTakeProfit()-Point)) takeProfitNeedsModify=true; }

      if(existingLockedProfit>=lockedProfit && !takeProfitNeedsModify) continue;
      ...
      if(orderType == OP_BUY) {
         if(!needStopLossModify) newStopLoss=OrderStopLoss();
         else {
            newStopLoss = OrderOpenPrice() + priceDistance; newStopLoss = NormalizeDouble(newStopLoss, Digits);
            if(OrderStopLoss() > 0 && newStopLoss <= OrderStopLoss()) continue;
            if(Bid - newStopLoss < stopLevel) { newStopLoss = Bid - stopLevel; newStopLoss = NormalizeDouble(newStopLoss, Digits); }
            if(newStopLoss <= 0 || newStopLoss >= Bid) continue;
            if(OrderStopLoss() > 0 && MathAbs(newStopLoss - OrderStopLoss()) < Point) continue;
         }
         ResetLastError();
      ...
      if(orderType == OP_SELL) {
         if(!needStopLossModify) newStopLoss=OrderStopLoss();
            ...
            newStopLoss = OrderOpenPrice() - priceDistance; newStopLoss = NormalizeDouble(newStopLoss, Digits);
            if(OrderStopLoss() > 0 && newStopLoss >= OrderStopLoss()) continue;
            if(newStopLoss - Ask < stopLevel) { newStopLoss = Ask + stopLevel; newStopLoss = NormalizeDouble(newStopLoss, Digits); }
            if(newStopLoss <= Ask) continue;
            if(OrderStopLoss() > 0 && MathAbs(newStopLoss - OrderStopLoss()) < Point) continue;
         }
         ResetLastError();
...
void CalculateSSL(int shift, double &sslUp, double &sslDown, int &hlv) {
   ...
   for(int i = oldest; i >= shift; i--) {
      ...
      if(candleClose > smaHigh) currentHlv = 1; else if(candleClose < smaLow) currentHlv = -1;
      if(i == shift) { hlv = currentHlv; sslUp = (currentHlv < 0) ? smaLow : smaHigh; sslDown = (currentHlv < 0) ? smaHigh : smaLow; return; }
   }
   hlv = currentHlv; sslUp = 0; sslDown = 0;
}

int GetCurrentSSLDirection() {
   if(Bars < SSLPeriod + 20) return 0;
   double up, down; int hlv; CalculateSSL(0, up, down, hlv);
   if(up > down) return 1; if(up < down) return -1;
   return 0;
}
...
bool IsBuySignal(int shift) {
   ...
   double upCurrent, downCurrent, upPrevious, downPrevious; int hlvCurrent, hlvPrevious;
   CalculateSSL(shift, upCurrent, downCurrent, hlvCurrent); CalculateSSL(shift + 1, upPrevious, downPrevious, hlvPrevious);
...
void DrawTrendSegment(string name, datetime time1, double price1, datetime time2, double price2, color lineColor) {
   ...
   ObjectSetInteger(0, name, OBJPROP_COLOR, lineColor); ObjectSetInteger(0, name, OBJPROP_WIDTH, SSLLineWidth);
   ObjectSetInteger(0, name, OBJPROP_RAY_RIGHT, false); ObjectSetInteger(0, name, OBJPROP_BACK, false);
...
void DrawHistoricalSignal(int shift, bool isBuy) {
   string type = isBuy ? "BUY" : "SELL"; string baseName = PREFIX + type + "_" + IntegerToString((int)Time[shift]);
   ...
   if(ShowSignalArrows) {
      ...
      ObjectSetInteger(0, arrowName, OBJPROP_ARROWCODE, isBuy ? 233 : 234); ObjectSetInteger(0, arrowName, OBJPROP_COLOR, isBuy ? BuyColor : SellColor);
      ObjectSetInteger(0, arrowName, OBJPROP_WIDTH, SignalArrowWidth); ObjectSetInteger(0, arrowName, OBJPROP_SELECTABLE, false);
   }
   if(ShowSignalText) {
      string textName = baseName + "_TEXT"; double textPrice = isBuy ? (price - SignalDistancePoints * 0.30 * Point) : (price + SignalDistancePoints * 0.30 * Point);
      ...
      ObjectSetString(0, textName, OBJPROP_TEXT, isBuy ? "Long +1" : "Short -1"); ObjectSetInteger(0, textName, OBJPROP_COLOR, isBuy ? BuyColor : SellColor);
      ObjectSetInteger(0, textName, OBJPROP_FONTSIZE, SignalFontSize); ObjectSetString(0, textName, OBJPROP_FONT, "Arial");
...
string TimeframeToString(int timeframe) {
   switch(timeframe) { case PERIOD_M1: return "M1"; case PERIOD_M5: return "M5"; case PERIOD_M15: return "M15"; case PERIOD_M30: return "M30"; case PERIOD_H1: return "H1"; case PERIOD_H4: return "H4"; case PERIOD_D1: return "D1"; case PERIOD_W1: return "W1"; case PERIOD_MN1: return "MN1"; } return "CURRENT";
}

void CreateDashboardPanel(string name, int x, int y, int width, int height, color background) {
   if(ObjectFind(0, name) < 0) ObjectCreate(0, name, OBJ_RECTANGLE_LABEL, 0, 0, 0);
   ObjectSetInteger(0, name, OBJPROP_CORNER, CORNER_RIGHT_UPPER); ObjectSetInteger(0, name, OBJPROP_XDISTANCE, x+10); ObjectSetInteger(0, name, OBJPROP_YDISTANCE, y);
   ...
   ObjectSetInteger(0, name, OBJPROP_BORDER_TYPE, BORDER_FLAT); ObjectSetInteger(0, name, OBJPROP_COLOR, clrDimGray); ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false); ObjectSetInteger(0, name, OBJPROP_HIDDEN, true);
}

void CreateDashboardLabel(string name, string text, int x, int y, int fontSize, color textColor) {
   if(ObjectFind(0, name) < 0) ObjectCreate(0, name, OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(0, name, OBJPROP_CORNER, CORNER_RIGHT_UPPER); ObjectSetInteger(0, name, OBJPROP_XDISTANCE, x); ObjectSetInteger(0, name, OBJPROP_YDISTANCE, y);
   ObjectSetString(0, name, OBJPROP_TEXT, text); ObjectSetString(0, name, OBJPROP_FONT, "Arial"); ObjectSetInteger(0, name, OBJPROP_FONTSIZE, fontSize);
   ObjectSetInteger(0, name, OBJPROP_COLOR, textColor); ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false); ObjectSetInteger(0, name, OBJPROP_HIDDEN, true);
}
...
int EMADirection=0; int GlobalBUYSELLdashboardScore=0;
...
void UpdateDashboard(DailyProtectionState &state) {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--) {
      ...
      if(type==OP_BUY || type==OP_SELL) {
         ...
         floatingProfit+=OrderProfit(); totalSwap+=OrderSwap(); totalCommission+=OrderCommission();
      } else pendingOrders++;
   }
   ...
   int currentSSLDirection=GetCurrentSSLDirection(); string sslDirection=currentSSLDirection>0?"BUY":currentSSLDirection<0?"SELL":"NONE";
   ...
   string emaState="N/A"; color emaColor=clrSilver;
   if(ema>0) { if(Bid>ema) { emaState="BULLISH"; emaColor=clrLime; EMADirection=1; } else if(Bid<ema) { emaState="BEARISH"; emaColor=clrTomato; EMADirection=-1; } else { emaState="AT EMA"; emaColor=clrGold; EMADirection=0; } }

   string statusText="READY"; color statusColor=clrLime;
   ...
   if(DayProfitLadderNextTargetEquity>DayProfitLadderProtectionEquity) {
      ...
      if(ladderProgress<0) ladderProgress=0; if(ladderProgress>100) ladderProgress=100;
   }
   ...
   double riskRemaining=BasketNewOrderLossLimitUSD>0 ? BasketNewOrderLossLimitUSD+netProfit : 0; if(riskRemaining<0) riskRemaining=0;

   string serverText="CONNECTED / OK"; color serverColor=clrLime;
   ...
   int h1Direction=GetH1Direction(); int m5Direction=GetM5Direction(); int dashboardDirection=(currentSSLDirection>0)?1:(currentSSLDirection<0?-1:0);
   string h1Text=h1Direction>0?"BUY":h1Direction<0?"SELL":"NONE"; string m5Text=m5Direction>0?"BUY":m5Direction<0?"SELL":"NONE";
   bool h1Confirm=(dashboardDirection!=0 && h1Direction==dashboardDirection); bool m5Confirm=(dashboardDirection!=0 && m5Direction==dashboardDirection);
   bool emaConfirm=(dashboardDirection!=0 && EMADirection==dashboardDirection); bool h1Opposite=(dashboardDirection!=0 && h1Direction==-dashboardDirection);

   double dashboardMomentumDiff=0.0; double dashboardMomentumThreshold=MathAbs(Min30MinutePriceDifference); bool momentumConfirm=false;
   if(dashboardDirection!=0) { dashboardMomentumDiff=Get30MinDifference(dashboardDirection==1?OP_BUY:OP_SELL); momentumConfirm=(dashboardDirection==1) ? (dashboardMomentumDiff>dashboardMomentumThreshold) : (dashboardMomentumDiff<-dashboardMomentumThreshold); }
   double dashboardATRRatio=GetM5CandleATRRatio(); bool dashboardExtremeVol=IsExtremeVolatility();
   ...
   string h1Mark=h1Confirm?"PASS":(h1Opposite?"OPPOSITE":"NO"); string m5Mark=m5Confirm?"PASS":"NO"; string emaMark=emaConfirm?"PASS":"NO"; string momMark=momentumConfirm?"PASS":"NO";
   ...
   string dplStatus = DayProfitLadderTradingStopped ? "STOPPED" : "TRADING"; color dplStatusColor = DayProfitLadderTradingStopped ? clrTomato : clrLime;
   ...
   int balancelomultipler=(int)(AccountBalance()/AccountMultiplierLOT); if(balancelomultipler<1) balancelomultipler=1;
...
void DeleteOurObjects() {
   ...
   for(int i = total - 1; i >= 0; i--) { string name = ObjectName(0, i, -1, -1); if(StringFind(name, PREFIX, 0) == 0) ObjectDelete(0, name); }
}

void DeleteDashboardObjects() {
   int total = ObjectsTotal(0, -1, -1);
   for(int i = total - 1; i >= 0; i--) { string name = ObjectName(0, i, -1, -1); if(StringFind(name, DASH_PREFIX, 0) == 0) ObjectDelete(0, name); }
}
...
void CreateLeftLivePanel(string name,int x,int y,int width,int height,color background) {
   if(ObjectFind(0,name)<0) ObjectCreate(0,name,OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,name,OBJPROP_CORNER,CORNER_LEFT_UPPER); ObjectSetInteger(0,name,OBJPROP_BACK,false); ObjectSetInteger(0,name,OBJPROP_ZORDER,1000);
   ObjectSetInteger(0,name,OBJPROP_XDISTANCE,x); ObjectSetInteger(0,name,OBJPROP_YDISTANCE,y); ObjectSetInteger(0,name,OBJPROP_XSIZE,width);
   ObjectSetInteger(0,name,OBJPROP_YSIZE,height); ObjectSetInteger(0,name,OBJPROP_BGCOLOR,background); ObjectSetInteger(0,name,OBJPROP_BORDER_TYPE,BORDER_FLAT);
   ObjectSetInteger(0,name,OBJPROP_COLOR,clrDimGray); ObjectSetInteger(0,name,OBJPROP_SELECTABLE,false); ObjectSetInteger(0,name,OBJPROP_HIDDEN,true);
}
...
void CreateLeftLiveLabel(string name,string text,int x,int y,int fontSize,color textColor) {
   ...
   ObjectSetString(0,name,OBJPROP_TEXT,text); ObjectSetString(0,name,OBJPROP_FONT,"Consolas"); ObjectSetInteger(0,name,OBJPROP_FONTSIZE,fontSize);
   ObjectSetInteger(0,name,OBJPROP_COLOR,textColor); ObjectSetInteger(0,name,OBJPROP_SELECTABLE,false); ObjectSetInteger(0,name,OBJPROP_HIDDEN,true);
}

void DeleteLeftLiveOrdersDashboardObjects() {
   int total=ObjectsTotal(0,-1,-1);
   for(int i=total-1; i>=0; i--) { string name=ObjectName(0,i,-1,-1); if(StringFind(name,LEFT_LIVE_PREFIX,0)==0) ObjectDelete(0,name); }
}

void UpdateLeftLiveOrdersDashboard() {
   int total=0,buyCount=0,sellCount=0,pendingCount=0; double buyLots=0,sellLots=0,netPL=0; int rows=LeftDashboardMaxRows;
   if(rows<1) rows=1; if(rows>24) rows=24;
   for(int i=OrdersTotal()-1; i>=0; i--) {
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) continue;
      if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MagicNumber) continue;
      int type=OrderType();
      if(type!=OP_BUY && type!=OP_SELL && type!=OP_BUYSTOP && type!=OP_SELLSTOP && type!=OP_BUYLIMIT && type!=OP_SELLLIMIT) continue;
      total++;
      if(type==OP_BUY) { buyCount++; buyLots+=OrderLots(); } else if(type==OP_SELL) { sellCount++; sellLots+=OrderLots(); } else pendingCount++;
   ...
   color pnlColor=clrWhite; if(netPL>0) pnlColor=clrLime; else if(netPL<0) pnlColor=clrTomato;
   ...
   for(int r=0; r<24; r++) { CreateLeftLiveLabel(LEFT_LIVE_PREFIX+"ROW"+IntegerToString(r), "", tx, y+108+(r*20), 8, clrWhite); CreateLeftLiveLabel(LEFT_LIVE_PREFIX+"VERIFY"+IntegerToString(r), "", tx+475, y+108+(r*20), 8, clrSilver); }
   ...
   for(int j=OrdersTotal()-1; j>=0; j--) {
      ...
      if(row>=rows) break;
      string typeText="";
      if(type==OP_BUY) typeText="BUY"; else if(type==OP_SELL) typeText="SELL"; else if(type==OP_BUYSTOP) typeText="BUY ST"; else if(type==OP_SELLSTOP) typeText="SELL ST"; else if(type==OP_BUYLIMIT) typeText="BUY LM"; else if(type==OP_SELLLIMIT) typeText="SELL LM";
      double lots=OrderLots(); double open=OrderOpenPrice(); double sl=OrderStopLoss(); double tp=OrderTakeProfit(); double pl=0;
      if(type==OP_BUY || type==OP_SELL) pl=OrderProfit()+OrderSwap()+OrderCommission();
      double slDiffPoints=0; if(sl>0) slDiffPoints=MathAbs(open-sl)/Point/100;
      double tpDiffPoints=0; if(tp>0) tpDiffPoints=MathAbs(tp-open)/Point/100;
      string slDiffText="-"; string tpDiffText="-";
      if(sl>0) slDiffText=DoubleToString(slDiffPoints,0); if(tp>0) tpDiffText=DoubleToString(tpDiffPoints,0);
      string plText="-"; if(type==OP_BUY || type==OP_SELL) plText=(pl>=0?"+":"")+DoubleToString(pl,2);
      string verifiedStatus="N/A"; color verifiedColor=clrSilver;
      if(type==OP_BUY || type==OP_SELL) {
         verifiedStatus=GetPostOrderSLTPVerificationStatus(OrderTicket());
         if(verifiedStatus=="VERIFIED") verifiedColor=clrLime; else if(verifiedStatus=="FAILED") verifiedColor=clrTomato; else if(verifiedStatus=="CHECKING") verifiedColor=clrGold; else if(verifiedStatus=="NOT CHECKED") verifiedColor=clrOrange;
      }
      ...
      if(type==OP_BUY) rowColor=clrDeepSkyBlue; else if(type==OP_SELL) rowColor=clrTomato; else rowColor=clrGold;
      if((type==OP_BUY || type==OP_SELL) && pl>0) rowColor=clrLime; if((type==OP_BUY || type==OP_SELL) && pl<0) rowColor=clrOrangeRed;
